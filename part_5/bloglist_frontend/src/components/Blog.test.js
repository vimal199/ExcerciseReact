...
test('like button is clicked twice, the event handler the component received as props is called twice', async () => {
    ...
    const { container } = render(<Blog blog={blog} user={user} handleClickLikes={mockHandler} />)
    const input_user = userEvent.setup()
...
test('form calls the event handler it received as props with the right details when a new blog is created', async () => {
    ...
    const { container } = render(<BlogForm title={blog.title} handleChangeTitle={mockHandlerForTitle} author={blog.author} handleChangeAuthor={mockHandlerForAuthor} url={blog.url} handleChangeUrl={mockHandlerForUrl} handleSubmitBlog={mockHandler}></BlogForm>)
    const input_user = userEvent.setup()
